...
class TestTrainingPbarControllPytorch:
    def _build_trainer(self, progress_info_controller):
        ...
        with patch('model_compression_toolkit.gptq.common.gptq_training.get_compare_points', return_value=([], None, [], [])), \
             patch('model_compression_toolkit.gptq.pytorch.gptq_training.get_gptq_trainable_parameters', return_value=([], [], [])), \
             patch('model_compression_toolkit.gptq.pytorch.gptq_training.get_weights_for_loss', return_value=([], [])), \
             patch.object(PytorchGPTQTrainer, 'build_gptq_model', return_value=_model_build_fn_return_value()):
            trainer = PytorchGPTQTrainer(graph_float=Mock(),
                                        graph_quant=Mock(),
        ...
        trainer.compute_gradients = Mock(return_value=(Mock(), []))
        return trainer
